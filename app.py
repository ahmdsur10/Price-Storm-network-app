...
# ─── بناء الخريطة (تعيد كائن folium.Map) ─────────────────────────────
def build_map(df, selected_indices=None, zoom_start=18):
...
# ─── تحويل الخريطة إلى HTML للعرض السريع ─────────────────────────────
def map_to_html(m):
...
with st.sidebar:
    ...
    st.markdown('<div style="text-align:center; margin-top:20px;"><b>Eng: Ahmed Adam</b><br>نظام تحليل شبكات السيول v6.0</div>', unsafe_allow_html=True)
...
# تبويب 1: خريطة ثابتة (سريعة جداً)
with tab1:
    ...
    with st.spinner("جاري تحميل الخريطة..."):
        ...
        html1 = map_to_html(m1)
    st.components.v1.html(html1, height=550)

# تبويب 2: اختيار خطوط وحساب التكلفة (خريطة ثابتة مع تحديث عند تغيير الاختيار)
with tab2:
    ...
    with col_right:
        with st.spinner("تحديث الخريطة..."):
            m2 = build_map(df, selected_indices=selected)
            html2 = map_to_html(m2)
        st.components.v1.html(html2, height=550)

# تبويب 3: رسم خط جديد (نستخدم st_folium لأنه يحتاج تفاعل)
with tab3:
    st.markdown('<div class="sec-title">ارسم خطاً على الخريطة</div>', unsafe_allow_html=True)
    col_draw, col_info = st.columns([3,2])
    with col_draw:
        m3 = build_map(df, zoom_start=18)
        # إضافة أداة الرسم إلى الخريطة
        Draw(draw_options={"polyline": {"shapeOptions": {"color": "#ff6b35", "weight": 4}}, 
                           "polygon": False, "rectangle": False, "circle": False, "marker": False}).add_to(m3)
        MeasureControl(position="topleft", primary_length_unit="meters").add_to(m3)
        output = st_folium(m3, width="100%", height=550, returned_objects=["all_drawings"], key="draw_map")
    with col_info:
